...
from ..observer import Subject, SubjectObserver
...
class _SimpleMixin:
    ...
    def output_cpu(self):
        ...
        if self._output_cpu is None:
            self._output_cpu = self._output_cuda.detach().cpu().numpy()
...
class SimpleValidator(_SimpleMixin, SubjectObserver):
...
class SimpleEvaluator(SubjectObserver):
    ...
    def _check_subject_type(self, subject):
        assert isinstance(subject, _SimpleMixin)
